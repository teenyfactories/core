...
def _log_embed_usage(provider: str, model: str, input_tokens: int, latency_ms: int, preview_text: str) -> None:
    """Internal: record an embedding usage row (verbatim — no cost). The
    orchestrator computes USD cost from `raw` at read time. Never raises."""
    try:
