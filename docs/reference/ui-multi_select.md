...
**NOT:** For single-selection (use `select` instead); for user-input strings (use `text_area` or `input`).
...
config:
  ...
  options_from: states | agents      # [dynamic] pull slugs from factory config
...
| `options_from` | enum | When dynamic | `'states'` or `'agents'` — pulls from factory config |
