...
## Data & events
No data binding. `tab` markers require a `title`, which becomes the tablist label. Panel containers hold any registered component (`layout_column`, `card`, `markdown`, etc.). Child `config.icon` is ignored (removed by product decision; retained for back-compat).
